...
// Handle global database wipe clear action
if (isset($_POST['action']) && $_POST['action'] === 'clear_ledger') {
    $conn->query("TRUNCATE TABLE orders");
    header("Location: orders.php");
    exit();
}

// AJAX: Toggle payment status on an order
if (isset($_POST['action']) && $_POST['action'] === 'toggle_payment') {
    $order_id = intval($_POST['order_id']);
    $paid_status = ($_POST['paid'] === '1') ? 1 : 0;
    $stmt = $conn->prepare("UPDATE orders SET paid = ? WHERE id = ?");
    $stmt->bind_param("ii", $paid_status, $order_id);
    $ok = $stmt->execute();
    $stmt->close();
...
// AJAX: Fetch order history for a specific table
if (isset($_GET['fetch']) && $_GET['fetch'] === 'table_history') {
    $table = $_GET['table'] ?? '';
    $filter = $_GET['filter'] ?? 'all'; // 'all' | 'unpaid'

    if ($table === 'Takeaway') {
        $where = "table_number = 'Takeaway'";
    } else {
        $t = intval($table);
        $where = "table_number = '$t'";
    }
    if ($filter === 'unpaid') {
        $where .= " AND (paid = 0 OR paid IS NULL) AND (status != 'canceled' OR status IS NULL)";
    }

    $result = $conn->query("SELECT * FROM orders WHERE $where ORDER BY id DESC");
    $rows = [];
    $total = 0;
    $unpaid_total = 0;
    if ($result && $result->num_rows > 0) {
        while ($r = $result->fetch_assoc()) {
            $rows[] = $r;
            if ((!isset($r['status']) || $r['status'] !== 'canceled')) {
                $total += $r['price'];
                if (empty($r['paid'])) $unpaid_total += $r['price'];
            }
        }
    }
    header('Content-Type: application/json');
    echo json_encode(['rows' => $rows, 'total' => $total, 'unpaid_total' => $unpaid_total]);
    $conn->close();
    exit();
}

// AJAX ENDPOINT FOR LIVE STREAM DATA DATA-ROWS
if (isset($_GET['fetch']) && $_GET['fetch'] === 'live_rows') {
    $result = $conn->query("SELECT * FROM orders ORDER BY id DESC");
    $total_orders = 0; $total_revenue = 0; $rows_html = '';
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $isCanceled = (isset($row['status']) && $row['status'] === 'canceled');
            $canceled_class = $isCanceled ? 'status-canceled' : '';
            $disabled_attr = $isCanceled ? 'disabled' : '';
            $delete_btn_attr = $isCanceled ? 'disabled style="opacity:0.3; cursor:not-allowed;"' : '';
            
            $location_badge = (isset($row['table_number']) && $row['table_number'] !== 'Takeaway') 
                ? '<span class="table-badge badge-qr">🪑 Table '.htmlspecialchars($row['table_number']).'</span>'
                : '<span class="table-badge badge-takeaway">🥡 Takeaway</span>';
                
            $note_badge = !empty($row['note']) 
                ? '<span class="note-pill">💡 '.htmlspecialchars($row['note']).'</span>' 
                : '—';
                
            $canceled_tag = $isCanceled ? ' <span class="badge-canceled-text">⚠️ CANCELED BY CUSTOMER</span>' : '';
            $formatted_price = number_format($row['price'], 2);

            // Generate options for tables 1 through 9 along with Takeaway option
            $current_val = $row['table_number'];
            $dropdown_options = '';
            
            // Takeaway option
            $selected_tk = ($current_val === 'Takeaway') ? 'selected' : '';
            $dropdown_options .= "<option value='Takeaway' {$selected_tk}>🥡 Takeaway</option>";
            
            // Tables 1 to 9
            for ($i = 1; $i <= 9; $i++) {
                $selected_tbl = ($current_val == $i) ? 'selected' : '';
                $dropdown_options .= "<option value='{$i}' {$selected_tbl}>🪑 Table {$i}</option>";
            }

            // Custom dropdown element that inherits identical styling to the badges
            $is_takeaway = ($current_val === 'Takeaway');
            $dropdown_style_class = $is_takeaway ? 'table-assign-dropdown badge-takeaway' : 'table-assign-dropdown badge-qr';

            $dropdown_html = "<select class='{$dropdown_style_class}' data-order-id='{$row['id']}' {$disabled_attr} onchange='updateDropdownClass(this)'>
                {$dropdown_options}
            </select>";

            // Payment status badge
            $is_paid = !empty($row['paid']) && $row['paid'] == 1;
            $paid_checked = $is_paid ? 'checked' : '';
            $paid_label_class = $is_paid ? 'paid-label paid' : 'paid-label unpaid';
            $paid_label_text = $is_paid ? '✅ Paid' : '💳 Unpaid';
            $paid_toggle = $isCanceled ? '' : "
            ...
            // Construct the identical table row markup structure natively
            $rows_html .= "<tr class='{$canceled_class}' data-order-id='{$row['id']}'>
                <td>{$location_badge}</td>
                <td>{$dropdown_html}</td>
                <td><strong class='item-name'>" . htmlspecialchars($row['item']) . "</strong>{$canceled_tag}</td>
                <td>{$note_badge}</td>
                ...
                <td>
                    <form class='ajax-cancel-form' style='display:inline;'>
                        <input type='hidden' name='action' value='cancel_order'>
                        <input type='hidden' name='order_id' value='{$row['id']}'>
                        <button type='submit' class='btn-cancel' {$delete_btn_attr}>Delete</button>
                    </form>
                </td>
            </tr>";
        }
    ...
    echo json_encode([
        "total" => $total_orders,
        "revenue" => "Active Earnings: Rs. " . number_format($total_revenue, 2),
        "html" => $rows_html
    ]);
    $conn->close();
    exit();
...
// Fallback logic to render the framework shell markup on the initial page load execution loop
$result = $conn->query("SELECT * FROM orders ORDER BY id DESC");
$total_orders = 0; $total_revenue = 0;
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        if (isset($row['status']) && $row['status'] === 'active') { $total_revenue += $row['price']; }
        $total_orders++;
    }
}
?>
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        /* Table Structure Rules with Global Center Alignment Setup */
        ...
        /* Highly Styled Table Reassignment Dropdown to match original badges */
        .table-assign-dropdown {
            ...
            /* Removes standard select arrows across systems for a gorgeous flat tag aesthetic */
            appearance: none;
        ...
        /* ── Table selector in modal trigger ── */
        .table-select-row {
            display: flex;
            align-items: center;
            gap: 10px;
...
<body>
<!-- New Order Sound & Banner Notification -->
...
<div class="container">
    <div class="admin-header">
        ...
        <div class="header-controls">
            ...
            <span class="stat-badge" id="badgeTotalOrders">Total: <?php echo $total_orders; ?></span>
            <span class="stat-badge" id="badgeTotalRevenue" style="color:#1d9e75; background:#e6fffa;">Active Earnings: Rs. <?php echo number_format($total_revenue,2); ?></span>
            
            <form method="POST" style="display:inline;" onsubmit="if(!confirm('Clear all orders?')) return false; localStorage.removeItem('cafe_done_checks'); return true;">
                <input type="hidden" name="action" value="clear_ledger">
                <button type="submit" class="btn-clear-data">Clear Table</button>
            </form>
...
<script>
    ...
    let _knownOrderCount = null; // null = first load, don't notify yet
    ...
    function _showNewOrderBanner() {
        ...
        // Force re-trigger animation even if banner is already visible
        banner.classList.remove('visible');
        // Flush the style so the browser registers the removal before re-adding
    ...
    // Checkbox state persisted in localStorage so page refresh doesn't lose them
    ...
    // Memory storage to keep track of dropdown selections during live background refreshes
    let activeDropdownSelections = {}; 

    // Helper script to dynamically adjust the colors of select tags on execution
    ...
    // Capture currently selected dropdown values before the server wipes them
    ...
    function syncLiveDataGridEngine() {
        captureCheckedStates();
        captureDropdownStates(); // Save selections before reloading the table HTML

        fetch('orders.php?fetch=live_rows')
        .then(res => res.json())
        .then(data => {
            _checkForNewOrders(data.total); // ── Notification check ──
            document.getElementById('badgeTotalOrders').textContent = "Total: " + data.total;
            document.getElementById('badgeTotalRevenue').textContent = data.revenue;
            ...
            tbody.querySelectorAll('tr').forEach(row => {
                ...
                // Restore Dropdown Selections & Apply proper styling colors instantly
        ...
        .catch(err => console.log("Real-time network sync pooling paused: " + err.message));
    }
    ...
    function bindPaidCheckboxEvents() {
        document.querySelectorAll('#liveOrdersTableBody .paid-chk').forEach(chk => {
            ...
            // Restore from cache to avoid flicker on live refresh
            ...
            chk.addEventListener('change', function() {
                ...
                fd.append('action', 'toggle_payment');
                fd.append('order_id', id);
                fd.append('paid', paid);
                fetch('orders.php', { method: 'POST', body: fd })
                    .then(r => r.json())
                    .catch(err => console.warn('Payment toggle error', err));
            });
        });
    }
    ...
    function _loadTableHistory() {
        ...
        fetch(`orders.php?fetch=table_history&table=${encodeURIComponent(_currentTableValue)}&filter=${_currentTableFilter}`)
            .then(r => r.json())
            .then(data => {
                _lastHistoryData = data;
                _renderModalRows(data);
            })
            .catch(() => {
    ...
    function _renderModalRows(data) {
        ...
        data.rows.forEach((r, i) => {
            ...
            const note = r.note ? `<span class="note-pill">💡 ${_esc(r.note)}</span>` : '—';
            const price = isCanceled
                ? `<span style="text-decoration:line-through;color:#94a3b8;">Rs. ${Number(r.price).toFixed(2)}</span>`
                : `Rs. ${Number(r.price).toFixed(2)}`;

            html += `<tr style="${isCanceled ? 'opacity:0.5' : ''}">
                <td style="color:#94a3b8;font-size:12px;">${i+1}</td>
                <td style="font-weight:600;">${_esc(r.item)}</td>
                <td>${note}</td>
    ...
    function printTableBill() {
        ...
        _lastHistoryData.rows.forEach(r => {
            ...
            const note = r.note ? `<br><small style="font-size:12px;font-weight:700;">${_esc(r.note)}</small>` : '';
            itemRows += `<tr>
                <td>${_esc(r.item)}${note}</td>
        ...
        <style>
            * { margin:0; padding:0; box-sizing:border-box; }
            ...
            /* Set page size to exactly the content height — browser measures it via JS below */
            @page {
                size: auto;
                margin: 8mm 10mm;
            }
        ...
        <script>
            // Dynamically set the @page size to exactly the rendered content height.
            // This is the only reliable way to make the print preview show zero blank space.
            (function () {
                function setExactPageHeight() {
                    var root = document.getElementById('receiptRoot');
                    if (!root) return;
                    // Total content height in mm
                    var heightPx = root.offsetHeight;
                    var heightMM = Math.ceil(heightPx * 25.4 / 96) + 4; // +4mm small safety buffer
                    var widthMM = 210; // A4 width — keeps horizontal layout stable
                    ...
                    style.textContent = '@page { size: ' + widthMM + 'mm ' + heightMM + 'mm; margin: 8mm 10mm; }';
                }
                window.addEventListener('load', setExactPageHeight);
            })();
        <\/script>
    ...
    function bindDeleteButtons() {
        document.querySelectorAll('.ajax-cancel-form').forEach(form => {
            ...
            form.addEventListener('submit', function(e) {
                e.preventDefault();
                if (!confirm("Are you sure you want to completely delete this record from database history logs?")) return;
                ...
                .then(data => {
                    if(data.status === 'success') {
                        ...
                        row.style.opacity = "0";
                        setTimeout(() => { syncLiveDataGridEngine(); }, 400);
    ...
    function bindTableChangeEvents() {
        document.querySelectorAll('.table-assign-dropdown').forEach(select => {
            select.addEventListener('change', function() {
                ...
                // Instantly update local states memory to prevent any 5-second race conditions
                ...
                formData.append('table_number', newTableValue);
                ...
                .then(data => {
                    if (data.status === 'success') {
                        // Success - Sync state changes smoothly
                        syncLiveDataGridEngine();
                    } else {
                        alert("Error updating table: " + (data.message || 'Unknown response'));
                    }
                })
                .catch(err => console.error("Database update sync failure: ", err));
            });
        });
    }

    /* 📊 EXPORT DAILY PDF REPORT GENERATOR ENGINE */
    function generateDailyReportPDF() {
        ...
        document.querySelectorAll('#liveOrdersTableBody tr').forEach(row => {
            ...
            if (itemEl) {
                ...
                // Parse strings like "Latte (x3), Black Coffee"
        ...
        // 3. Build a beautiful list of active orders for the document summary section
        let ordersListHtml = "";
        document.querySelectorAll('#liveOrdersTableBody tr').forEach(row => {
            if (row.classList.contains('status-canceled')) return;
            
            // Extract Badge Table context
            ...
            // Check index columns to find exact values
        ...
        // 4. Open clean sandbox print window designed strictly for pdf styling exports
        const reportHtml = `
            <html>
            <head>
                ...
                <style>
                    ...
                    @media print {
                        ...
                        /* Prevent stray content (a lone row/footer) from spilling onto a second, mostly-blank page */
    ...
    syncLiveDataGridEngine();
    setInterval(syncLiveDataGridEngine, 1000);
</script>
</body>
</html>